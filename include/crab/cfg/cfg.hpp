...
namespace cfg {
    ...
    enum stmt_code {
      ...
      // functions calls
      CALLSITE = 50, RETURN = 51, CRAB_INTRINSIC = 52,
  ...
  void write(crab_os &o) const {
    ...
    class statement {
      ...
      bool is_intrinsic() const {
        return (m_stmt_code == CRAB_INTRINSIC);
      }
    ...
    */
    ...
    class bool_assign_cst: public statement<Number, VariableName> {
     ...
     public:
      ...
      bool_assign_cst(variable_t lhs, linear_constraint_t rhs): statement_t(BOOL_ASSIGN_CST), m_lhs(lhs), m_rhs(rhs) {      
        this->m_live.add_def(m_lhs);
        for(auto v: m_rhs.variables()) 
          this->m_live.add_use(v);
      }
...
public:
  ...
  // void - void callsite (hack for special calls)
  const statement_t* callsite(std::string func) { 
  ...
  //CLAM_VAR_TAGS callsite : used to project lin_cst on specific vars
